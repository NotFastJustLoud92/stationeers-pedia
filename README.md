...
Gas-property pages (Oxygen, Nitrogen, etc.) have full spec data — Specific Heat, Latent Heat, freeze/boil points, Combustion reactions — but no icon. Icons are matched by `Prefab Hash`, and pure gas types aren't spawnable `Thing` prefabs the way physical items/structures are, so there's no `Thing.Thumbnail` sprite to extract for them. The in-game SPDA sources gas icons from a separate `Stationpedia._gasThumbnails` list instead, which stays empty no matter what's tried to force it (`PopulateGases()`, `Initialize()`, `Open()`, `OnEnable()`) — reflection even shows the live client missing a method/field that static analysis of the compiled game assembly confirms exists. Ruled out a client/server version or branch mismatch as the cause (both confirmed on the same default-branch build). The actual mechanism remains unclear; not pursued further as a one-off nice-to-have.

...
Gas-property pages (Oxygen, Nitrogen, etc.) don't have a real extracted sprite icon. Icons for physical items are matched by `Prefab Hash`, and pure gas types aren't spawnable `Thing` prefabs, so there's no `Thing.Thumbnail` to extract. The in-game SPDA sources gas icons from a separate `Stationpedia._gasThumbnails` list, which stayed empty no matter what was tried to force it (`PopulateGases()`, `Initialize()`, `Open()`, `OnEnable()`) — even ruled out a client/server version or branch mismatch as the cause, and the real mechanism stayed unclear. Rather than keep chasing real sprite extraction, `search.html` instead generates a colored chemical-formula badge for every gas page (e.g. a red "CH4" diamond for Methane), matching the in-game SPDA's own visual style for these — see the `GAS_INFO` table in `tools/search_template.html`.

...
Gas-property pages (Oxygen, Nitrogen, etc.) have full spec data — Specific Heat, Latent Heat, freeze/boil points, Combustion reactions — but no icon. Icons are matched by `Prefab Hash`, and pure gas types aren't spawnable `Thing` prefabs the way physical items/structures are, so there's no `Thing.Thumbnail` sprite to extract for them. The in-game SPDA sources gas icons from a separate `Stationpedia._gasThumbnails` list instead. Investigated trying to force that list to populate (it stays empty even on a real client, not just the headless server) — reflection diagnostics showed the running client's `Stationpedia` class doesn't even have a method or field by the expected names, which points to a game-version mismatch between the client used to capture data and these dedicated servers (Steam auto-updates a client immediately; server updates here are manual/scheduled) rather than a fixable extraction bug. Not pursued further - would require reconciling client/server versions first, a separate and bigger problem than icon extraction itself.

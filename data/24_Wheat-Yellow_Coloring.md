# Wheat - Yellow Coloring

## Wheat

- **Key:** ReagentWheat
- **Reagents Type:** Wheat
**Found In Ore:**
  - NameOfThing=Wheat, QuantityOfThing=1

---

## Wheat Seeds

Grow some Wheat.

- **Key:** ThingSeedBag_Wheet
- **Prefab Name:** SeedBag_Wheet
- **Prefab Hash:** -654756733
- **Memory Size:** 0 B
- **Memory Access:** None
- **Nutrition:** 0
- **Nutrition Quality:** Low (-25% hydration capacity)
- **Stack Size:** 10
- **Paintable:** No
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)
**Found In Fermentation:**
  - NameOfThing=Liquid Alcohol, QuantityOfThing=6 mol
  - NameOfThing=Polluted Water, QuantityOfThing=0.3 mol
**Used In:**
  - NameOfThing=Fermenter, PrefabHash=1103525139, PageLink=ThingStructureFermenter

---

## White Bishop

- **Key:** ThingWhiteBishop
- **Prefab Name:** WhiteBishop
- **Prefab Hash:** -1560617987
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## White King

- **Key:** ThingWhiteKing
- **Prefab Name:** WhiteKing
- **Prefab Hash:** 264282242
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## White Knight

- **Key:** ThingWhiteKnight
- **Prefab Name:** WhiteKnight
- **Prefab Hash:** 1258419261
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## White Pawn

- **Key:** ThingWhitePawn
- **Prefab Name:** WhitePawn
- **Prefab Hash:** 1679241240
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## White Queen

- **Key:** ThingWhiteQueen
- **Prefab Name:** WhiteQueen
- **Prefab Hash:** 1968007961
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## White Rook

- **Key:** ThingWhiteRook
- **Prefab Name:** WhiteRook
- **Prefab Hash:** 921811279
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 100 x Energy + 0.5 x Silicon (from Ingot (Silicon))


---

## Wind Turbine

The Wind Turbine is optimized to produce power (up to 500W) even on low atmosphere worlds, it performs best in denser environments. Output varies with wind speed and, during storms, may increase dramatically (up to 20,000W), so be careful to design your power networks with that in mind.

- **Key:** ThingStructureWindTurbine
- **Prefab Name:** StructureWindTurbine
- **Prefab Hash:** -2082355173
- **Power Generation:** 1 kW
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)
**Build States:**
  - Description=1 x Kit (Wind Turbine)
  - Description=Welding Torch 5 x Steel Sheets
  - Description=5 x Cable Coil
  - Description=Screwdriver
**Connections:**
  - LogicName=Connection, LogicAccessTypes=0
  - LogicName=Connection, LogicAccessTypes=1
**Constructed Things:**
  - NameOfThing=Kit (Wind Turbine), PrefabHash=-868916503, PageLink=ThingItemKitWindTurbine

---

## Window (Base Cap 1)

- **Key:** ThingStuctureCompositeWindowRoundedBaseCap1
- **Prefab Name:** StuctureCompositeWindowRoundedBaseCap1
- **Prefab Hash:** 517334352
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Base Cap 2)

- **Key:** ThingStuctureCompositeWindowRoundedBaseCap2
- **Prefab Name:** StuctureCompositeWindowRoundedBaseCap2
- **Prefab Hash:** -2015578902
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Base Cutouts)

- **Key:** ThingStuctureCompositeWindowRoundedBaseLogic
- **Prefab Name:** StuctureCompositeWindowRoundedBaseLogic
- **Prefab Hash:** 1682495797
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Base)

- **Key:** ThingCompositeWindow2RoundedBase
- **Prefab Name:** CompositeWindow2RoundedBase
- **Prefab Hash:** -1686401318
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - StationBuildCostInsert
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Base)

- **Key:** ThingStuctureCompositeWindowRoundedBase
- **Prefab Name:** StuctureCompositeWindowRoundedBase
- **Prefab Hash:** -566986974
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Cap 1)

- **Key:** ThingStructureCompositeWindowTubeCap1
- **Prefab Name:** StructureCompositeWindowTubeCap1
- **Prefab Hash:** 806317173
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Cap 2)

- **Key:** ThingStructureCompositeWindowTubeCap2
- **Prefab Name:** StructureCompositeWindowTubeCap2
- **Prefab Hash:** -1459208753
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Corner 2)

- **Key:** ThingStructureCompositeWindowRoundedCorner2
- **Prefab Name:** StructureCompositeWindowRoundedCorner2
- **Prefab Hash:** -1371791233
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Corner)

- **Key:** ThingStructureCompositeWindowRoundedCorner
- **Prefab Name:** StructureCompositeWindowRoundedCorner
- **Prefab Hash:** -501201412
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Cupola)

- **Key:** ThingStructureCompositeWindowRoundedCupola
- **Prefab Name:** StructureCompositeWindowRoundedCupola
- **Prefab Hash:** 919048948
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Panel 1)

- **Key:** ThingStructureCompositeWindowRoundedPanel1
- **Prefab Name:** StructureCompositeWindowRoundedPanel1
- **Prefab Hash:** -253810931
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Panel 2)

- **Key:** ThingStuctureCompositeWindowRoundPanel
- **Prefab Name:** StuctureCompositeWindowRoundPanel
- **Prefab Hash:** 112142111
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Window (Round)

- **Key:** ThingStuctureCompositeWindowRound
- **Prefab Name:** StuctureCompositeWindowRound
- **Prefab Hash:** 539675452
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
- **Auto Ignition:** 573 K (300oC)
**Build States:**
  - Description=1 x Kit (T.U.B.E.)
**Constructed Things:**
  - NameOfThing=Kit (T.U.B.E.), PrefabHash=-344057907, PageLink=ThingItemKitTUBE

---

## Winterspawn (Alpha strain)

Agrizero's Winterspawn atmospheric bio-processor is a recent addition to their catalog of genespliced environmental decorations. Using ambient heat to split Water into Methane and Oxygen, the Winterspawn cools its surroundings, when supplied with sufficient Nitrogen. The alpha variant has a peak cooling and electrolysis capacity of 90Watts and is most efficient operating in air temperatures of 0 to 40 Degrees Celsius.

- **Key:** ThingItemPlantEndothermic_Genepool1
- **Prefab Name:** ItemPlantEndothermic_Genepool1
- **Prefab Hash:** 851290561
- **Memory Size:** 0 B
- **Memory Access:** None
- **Nutrition:** 0
- **Nutrition Quality:** Low (-25% hydration capacity)
- **Growth Time:** 36min 1s
- **Stack Size:** 10
- **Paintable:** No
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 1.47 kK (1200oC)
**Life Requirements:**
  - Name=Inhaled Gasses, Value=Nitrogen 21.6 Moles per hour , Gene=Gas production, ValueSize=18
  - Name=Exhaled Gasses, Value=Oxygen 1.44 Moles per hour Methane 2.88 Moles per hour , Gene=Gas production, ValueSize=12
  - Name=Toxic Gasses, Value=Pollutant, Methane, Silanol, Hydrochloric Acid, Hydrazine, , Gene=Toxins resistance, ValueSize=18
  - Name=Water usage, Value=1.44 Moles per hour, Gene=Water usage, ValueSize=18
  - Name=Min grow temperature, Value=0°C, Gene=Low temperature resistance, ValueSize=18
  - Name=Max grow temperature, Value=50°C, Gene=High temperature resistance, ValueSize=18
  - Name=Min ideal grow temperature, Value=20°C, Gene=Low temperature resistance, ValueSize=18
  - Name=Max ideal grow temperature, Value=30°C, Gene=High temperature resistance, ValueSize=18
  - Name=Min grow pressure, Value=25kPa, Gene=Low pressure resistance, ValueSize=18
  - Name=Max grow pressure, Value=200kPa, Gene=High pressure resistance, ValueSize=18
  - Name=Min ideal grow pressure, Value=50kPa, Gene=Low pressure resistance, ValueSize=18
  - Name=Max ideal grow pressure, Value=100kPa, Gene=High pressure resistance, ValueSize=18
  - Name=Light per day, Value=10min 0s, Gene=Light per day, ValueSize=18
  - Name=Darkness per day, Value=5min 0s, Gene=Darkness per day, ValueSize=18
  - Name=Time until toxin damage, Value=5min 0s, Gene=Toxins tolerance, ValueSize=18
  - Name=Time until drought damage, Value=30min 0s, Gene=Drought tolerance, ValueSize=18
  - Name=Time until frozen damage, Value=5min 0s, Gene=Low temperature tolerance, ValueSize=18
  - Name=Time until overheat damage, Value=5min 0s, Gene=High temperature tolerance, ValueSize=18
  - Name=Time until suffocate damage, Value=1h 0min 0s, Gene=Suffocation tolerance, ValueSize=18
  - Name=Time until low pressure damage, Value=10min 0s, Gene=Low pressure tolerance, ValueSize=18
  - Name=Time until high pressure damage, Value=10min 0s, Gene=High pressure tolerance, ValueSize=18
  - Name=Time until light damage, Value=1h 0min 0s, Gene=Light tolerance, ValueSize=18
  - Name=Time until darkness damage, Value=1h 0min 0s, Gene=Darkness tolerance, ValueSize=18
**Constructed From Kits:**
  - NameOfThing=Winterspawn Seed (Alpha strain), PrefabHash=-1740754636, PageLink=ThingSeedBag_WinterspawnAlpha
**Found In Fermentation:**
  - NameOfThing=Liquid Alcohol, QuantityOfThing=0 mol
  - NameOfThing=Polluted Water, QuantityOfThing=0 mol
**Used In:**
  - NameOfThing=Fermenter, PrefabHash=1103525139, PageLink=ThingStructureFermenter

---

## Winterspawn (Beta strain)

Agrizero's Winterspawn atmospheric bio-processor is a recent addition to their catalog of genespliced environmental decorations. Using ambient heat to split Water into Methane and Oxygen, the Winterspawn cools its surroundings when supplied with sufficient Nitrogen. The beta variant has a peak cooling and electrolysis capacity of 150Watts and is most efficient operating in air temperatures of 14 to 24 Degrees Celsius.

- **Key:** ThingItemPlantEndothermic_Genepool2
- **Prefab Name:** ItemPlantEndothermic_Genepool2
- **Prefab Hash:** -1414203269
- **Memory Size:** 0 B
- **Memory Access:** None
- **Nutrition:** 0
- **Nutrition Quality:** Low (-25% hydration capacity)
- **Growth Time:** 1h 0min 1s
- **Stack Size:** 10
- **Paintable:** No
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 1.47 kK (1200oC)
**Life Requirements:**
  - Name=Inhaled Gasses, Value=Nitrogen 36 Moles per hour , Gene=Gas production, ValueSize=18
  - Name=Exhaled Gasses, Value=Oxygen 2.88 Moles per hour Methane 5.76 Moles per hour , Gene=Gas production, ValueSize=12
  - Name=Toxic Gasses, Value=Pollutant, Methane, Silanol, Hydrochloric Acid, Hydrazine, , Gene=Toxins resistance, ValueSize=18
  - Name=Water usage, Value=2.88 Moles per hour, Gene=Water usage, ValueSize=18
  - Name=Min grow temperature, Value=0°C, Gene=Low temperature resistance, ValueSize=18
  - Name=Max grow temperature, Value=50°C, Gene=High temperature resistance, ValueSize=18
  - Name=Min ideal grow temperature, Value=20°C, Gene=Low temperature resistance, ValueSize=18
  - Name=Max ideal grow temperature, Value=30°C, Gene=High temperature resistance, ValueSize=18
  - Name=Min grow pressure, Value=25kPa, Gene=Low pressure resistance, ValueSize=18
  - Name=Max grow pressure, Value=200kPa, Gene=High pressure resistance, ValueSize=18
  - Name=Min ideal grow pressure, Value=50kPa, Gene=Low pressure resistance, ValueSize=18
  - Name=Max ideal grow pressure, Value=100kPa, Gene=High pressure resistance, ValueSize=18
  - Name=Light per day, Value=10min 0s, Gene=Light per day, ValueSize=18
  - Name=Darkness per day, Value=5min 0s, Gene=Darkness per day, ValueSize=18
  - Name=Time until toxin damage, Value=5min 0s, Gene=Toxins tolerance, ValueSize=18
  - Name=Time until drought damage, Value=30min 0s, Gene=Drought tolerance, ValueSize=18
  - Name=Time until frozen damage, Value=5min 0s, Gene=Low temperature tolerance, ValueSize=18
  - Name=Time until overheat damage, Value=5min 0s, Gene=High temperature tolerance, ValueSize=18
  - Name=Time until suffocate damage, Value=1h 0min 0s, Gene=Suffocation tolerance, ValueSize=18
  - Name=Time until low pressure damage, Value=10min 0s, Gene=Low pressure tolerance, ValueSize=18
  - Name=Time until high pressure damage, Value=10min 0s, Gene=High pressure tolerance, ValueSize=18
  - Name=Time until light damage, Value=1h 0min 0s, Gene=Light tolerance, ValueSize=18
  - Name=Time until darkness damage, Value=1h 0min 0s, Gene=Darkness tolerance, ValueSize=18
**Constructed From Kits:**
  - NameOfThing=Winterspawn Seed (Beta strain), PrefabHash=-475065916, PageLink=ThingSeedBag_WinterspawnBeta
**Found In Fermentation:**
  - NameOfThing=Liquid Alcohol, QuantityOfThing=0 mol
  - NameOfThing=Polluted Water, QuantityOfThing=0 mol
**Used In:**
  - NameOfThing=Fermenter, PrefabHash=1103525139, PageLink=ThingStructureFermenter

---

## Winterspawn Seed (Alpha strain)

- **Key:** ThingSeedBag_WinterspawnAlpha
- **Prefab Name:** SeedBag_WinterspawnAlpha
- **Prefab Hash:** -1740754636
- **Memory Size:** 0 B
- **Memory Access:** None
- **Nutrition:** 0
- **Nutrition Quality:** Low (-25% hydration capacity)
- **Stack Size:** 10
- **Paintable:** No
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)
**Found In Fermentation:**
  - NameOfThing=Liquid Alcohol, QuantityOfThing=6 mol
  - NameOfThing=Polluted Water, QuantityOfThing=0.3 mol
**Used In:**
  - NameOfThing=Fermenter, PrefabHash=1103525139, PageLink=ThingStructureFermenter

---

## Winterspawn Seed (Beta strain)

- **Key:** ThingSeedBag_WinterspawnBeta
- **Prefab Name:** SeedBag_WinterspawnBeta
- **Prefab Hash:** -475065916
- **Memory Size:** 0 B
- **Memory Access:** None
- **Nutrition:** 0
- **Nutrition Quality:** Low (-25% hydration capacity)
- **Stack Size:** 10
- **Paintable:** No
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)
**Found In Fermentation:**
  - NameOfThing=Liquid Alcohol, QuantityOfThing=6 mol
  - NameOfThing=Polluted Water, QuantityOfThing=0.3 mol
**Used In:**
  - NameOfThing=Fermenter, PrefabHash=1103525139, PageLink=ThingStructureFermenter

---

## Wire Cutters

Wirecutters allow you to deconstruct various structures, as well as cross-lay cables when held in your non-active hand, and defuse explosives as needed. Wirecutters are stored in the Tool Belt, along with other essential tools.

- **Key:** ThingItemWireCutters
- **Prefab Name:** ItemWireCutters
- **Prefab Hash:** 1535854074
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
- **Auto Ignition:** 523 K (250oC)
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 500 x Energy + 3 x Iron (from Ingot (Iron))


---

## Wireless Battery Cell Extra Large

- **Key:** ThingItemWirelessBatteryCellExtraLarge
- **Prefab Name:** ItemWirelessBatteryCellExtraLarge
- **Prefab Hash:** -504717121
- **Power Storage:** 144 kW
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)
**Mode Options:**
  - LogicName=Empty, LogicAccessTypes=0
  - LogicName=Critical, LogicAccessTypes=1
  - LogicName=VeryLow, LogicAccessTypes=2
  - LogicName=Low, LogicAccessTypes=3
  - LogicName=Medium, LogicAccessTypes=4
  - LogicName=High, LogicAccessTypes=5
  - LogicName=Full, LogicAccessTypes=6

---

## Wreckage

- **Key:** ThingItemWreckageLargeExtendableRadiator
- **Prefab Name:** ItemWreckageLargeExtendableRadiator
- **Prefab Hash:** -964363642
- **Memory Size:** 0 B
- **Memory Access:** None
- **Stack Size:** 10
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)

---

## Wreckage

- **Key:** ThingItemWreckageSmallFan
- **Prefab Name:** ItemWreckageSmallFan
- **Prefab Hash:** -2120466121
- **Memory Size:** 0 B
- **Memory Access:** None
- **Stack Size:** 10
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)

---

## Wreckage

- **Key:** ThingItemWreckageSteelFragmentSmall
- **Prefab Name:** ItemWreckageSteelFragmentSmall
- **Prefab Hash:** -178735543
- **Memory Size:** 0 B
- **Memory Access:** None
- **Stack Size:** 10
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)

---

## Wreckage

- **Key:** ThingItemWreckageStructureWeatherStation
- **Prefab Name:** ItemWreckageStructureWeatherStation
- **Prefab Hash:** 1013894259
- **Memory Size:** 0 B
- **Memory Access:** None
- **Stack Size:** 10
- **Paintable:** Yes
- **Flashpoint:** 373 K (100oC)
- **Auto Ignition:** 573 K (300oC)

---

## Wrench

One of humanity's enduring contributions to the cosmos, the wrench represents the essence of our species. A simple, effective and spiritually barren tool, use it to build and deconstruct a variety of structures

- **Key:** ThingItemWrench
- **Prefab Name:** ItemWrench
- **Prefab Hash:** -1886261558
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
- **Auto Ignition:** 523 K (250oC)
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 500 x Energy + 3 x Iron (from Ingot (Iron))


---

## Xigo

A 2150CE survey revealed the Xigo 'X' as one of the most recognized symbols in the world. Yet few participants could clearly describe the company's actual scope. Precisely where the company begins and ends has been the subject of endless theories, financial studies and lawsuits – originally a website for price comparisons, the conglomerate expanded to embrace data, then distribution and finally production. It was a seemingly indiscriminate expansion, focused not on one industry, but all of them – taking in so many fields that business analysts have largely rejected the idea of Xigo as a single entity. Rather, it is a conceptual grouping for a branch of human activity. And while such statistics are – by their very nature – difficult to confirm, recent estimates suggest Xigo employs over 1.5% of the entire human race. Suffice to say, it produces almost everything.

- **Key:** Xigo

---

## Yellow Coloring

- **Key:** ReagentYellow Coloring
- **Unit:** g

---


...
**Logic Slot Types:**
  ...
  - LogicName=Open, LogicAccessTypes=1
  - LogicName=On, LogicAccessTypes=1
  - LogicName=Lock, LogicAccessTypes=1
...
Displays Setting between 0 and 1 on a lit up bar. Has an adjustable Color.

- **Key:** ThingModularDeviceSliderDiode1
- **Prefab Name:** ModularDeviceSliderDiode1
- **Prefab Hash:** 2015129381
- **Base Power Draw:** 10 W
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
...
- **Key:** ThingModularDeviceSliderDiode2
- **Prefab Name:** ModularDeviceSliderDiode2
- **Prefab Hash:** -518701409
- **Base Power Draw:** 10 W
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** Yes
...
After the patents for several delivery robots expired, Norsec's robotic division was quick to aquire, repurpose, and augment these designs for the Stationeer program. The DiRCI (or Directed Relocatable Cargo Interface) was the output of the amalgamation of these patents and Norsec insanity. DiRCI is designed for long range item transport and requires programming knowledge to operate. DiRCI has 8 modes: 0 = Idle = Do nothing 1 = MoveToTarget = Move to target in straight line, returns to Idle when reached 2 = LoadCargo = Pull items from the Vending Machine, Vending Machine Small, or Refrigerated Vending Machine it is facing 3 = UnloadCargo = Unload in chute input or chute bin within 2 meters / 1.25 large grids 4 = CargoFull = Automaticly assigned if full while trying to load Cargo 5 = CargoEmpty = Automaticly assigned if empty while trying to unload Cargo 6 = PathFromStack = See Below 7 = StackPointArrived = Automaticly assigned, used for telling when to move through the stack PathFromStack: A path point is stored in the stack in four addresses. [PathID] - Must be greater than zero [TargetX] - This point's X position [TargetY] - This point's Y position [TargetZ] - This point's Z position Recommendation: Clear the stack before writing to it and use the put instruction for writing path points to maintain alignment. The absolute value of Setting minus one is the stack position. (1 or -1 is position 0 in the stack) #1. When PathFromStack is assigned from any mode below 4, check the stack at the stack position for PathID, if it is not valid, go to Idle. #2. If it is valid store it, then read up and set TargetX, TargetY, TargetZ and begin moving. #3. When StackPointArrived is assigned, check 4 up if Setting was positive or 4 down if negative. #4. If the value at the checked address equals the stored PathID, jump to its position and goto step #2. #5. Othersize go to Idle. Important notes: *Will move to Idle if LoadCargo or UnloadCargo do not have a target. *When its checking the stack and would go out of range, it will move to Idle. *Assigning mode 6 with Setting at zero will move it to Idle. *DiRCI's stack can hold 65 entries, or 16 points. *The stack cannot be interacted with via transmitter, it must be interfaced via the IC10 socket. *Stack position 64 is used for overriding max speed. its needs to be greater than 0.1 to take affect and capped at 6. Connects to Logic Transmitter

- **Key:** ThingrobotDiRCI
- **Prefab Name:** robotDiRCI
- **Prefab Hash:** -1117847913
- **Memory Size:** 520 B
...
**Mode Options:**
  ...
  - LogicName=MoveToTarget, LogicAccessTypes=1
  - LogicName=LoadCargo, LogicAccessTypes=2
  - LogicName=UnloadCargo, LogicAccessTypes=3
  - LogicName=CargoFull, LogicAccessTypes=4
  - LogicName=CargoEmpty, LogicAccessTypes=5
  - LogicName=PathFromStack, LogicAccessTypes=6
  - LogicName=StackPointArrived, LogicAccessTypes=7
**Constructed Things:**
  - NameOfThing=Kit (DiRCI), PrefabHash=-1373381823, PageLink=ThingItemDiRCIkit
...
- **Key:** ThingEarMuffs
- **Prefab Name:** EarMuffs
- **Prefab Hash:** 621244184
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build Recipe:**
  - Printer: Tool Manufactory (Tier One)
    - 500 x Energy + 10 x Iron (from Ingot (Iron)) + 15 x Silicon (from Ingot (Silicon))
...
- **Key:** ThingModularDeviceEmergencyButton3x3
- **Prefab Name:** ModularDeviceEmergencyButton3x3
- **Prefab Hash:** 2088517573
- **Memory Size:** 0 B
- **Memory Access:** None
- **Paintable:** No
**Build States:**
